...
> 使用方式：这是查漏清单，不是逐条必问题卷。先从用户输入、代码、接口、现有文档中预填；只问会影响方案方向、数据状态、权限、接口契约或验收标准的阻塞项。低风险未知写明假设即可继续。
...
按「十一、实现 Todo」逐项完成，每完成一项运行对应验证；遇到影响状态流转、权限、数据归属、接口契约、DB 结构或回滚语义的待确认项时先暂停询问，低风险假设按文档记录执行。"
