...
复杂度可按改动范围、模块数量、接口数量和代码证据判断时直接写入；只有判断会影响模型/拆分策略时才确认。**先根据 `$task` 和 Step 1 的上下文给出建议值**，让用户确认而不是凭空估算：
...
**规则：先把用户已说清楚的内容、代码里能确定的入口/字段/状态、现有文档里的约束填入草稿；不要机械按问题集逐条问。确需询问时每次只问一个，并说明为什么这个答案会影响方案。**
...
- **显式暴露需求冲突**：如果用户需求与现有代码路径、状态机、字典值、权限模型、数据归属或表复用方式冲突，单独写「需求冲突/待确认」小节，给出证据、风险和建议口径；不要静默折中
...
- [ ] $task 已确认（不为空）
- [ ] 任务类型和复杂度已确认
- [ ] 问题集已用于查漏；阻塞项已确认，非阻塞未知已标注假设或 `待补充`
- [ ] 文件路径冲突已处理
- [ ] 不相关章节已删除（避免大量"待补充"）
- [ ] 最小影响分析已包含
