...
| “基于这个 yan-dev-doc 生成多 AI review 任务包” | `review-fix` |
...
- 归一化前检查 ID 唯一性：同一 ID 对应不同根因时标 `Blocked` 并要求重编；同一根因重复出现时合并来源，但保留所有原始来源编号。
...
nextCommand: <有 blocked：人工确认；有 deferred-next-batch：使用 yan-code-review skill，mode=repair，继续下一批；只有普通 deferred 或无未关闭 CR/IM：使用 yan-project-analysis skill，mode=understanding，继续，普通 deferred 不自动重跑>
...
- <命令>：<通过/失败/未运行 + 摘要>
- TestDependencyClass：<Hermetic / ServiceBacked / LiveExternal / Mixed / Unknown；说明默认 CI 与外部测试边界>
- TestEvidenceStatus：<Passed / Failed / NotRun / EnvironmentBlocked；环境阻塞时写工具链版本>
...
| Prompt | 期望 |
|--------|------|
| `使用 yan-code-review skill，mode=repair，按这些 findings 直接修复：CR-1 ...` | 触发，直接修复 |
...
| `基于 yan-dev-doc 生成 review 任务包` | 不触发，应该用 `review-fix` |
