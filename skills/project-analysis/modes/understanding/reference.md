...
nextCommand: <需要落方案：使用 dev-doc skill 基于本影响分析生成开发方案；需要缺陷判断：使用 code-review skill，mode=check，基于本影响分析执行只读审查；否则人工：确认影响结论>
...
> 本节只记录结构性偏差和观察，不定性为 Bug；是否构成问题交给 `code-review mode=check` 判断。
...
nextCommand: 人工：按代码地图完成 Review 和 Submit Gate 检查；发现新风险时使用 code-review skill，mode=check，重新审查
