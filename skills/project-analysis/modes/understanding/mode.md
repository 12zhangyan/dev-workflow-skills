---
name: code-reading
description: 在 Code Review 前生成开发者代码地图，或只读分析新旧接口契约、现有调用链和兼容性影响；CodeMap 生成 md 并登记看板，ImpactAnalysis 仅聊天输出且仓库零写入。由 project-analysis 根入口的 understanding mode 加载。
...
同时遵循 [../../../_shared/workflow-gates.md](../../../_shared/workflow-gates.md)：本 skill 服务 Understanding Gate；`CodeMap` 输出代码地图产物，`ImpactAnalysis` 明确写 `artifacts: 无（聊天只读分析）`；两者都给出人工 review 入口，以及如需 AI 审查时回到 `code-review mode=package` / `code-review mode=check` 的路径。
...
可以进入人工 Review / 提交前检查。
如仍需要 AI 审查，先使用 `code-review mode=package` 生成任务包，并让 `mode=check` 输出 findings；不要用代码地图替代审查结论。
```
...
- **不越界**：不提修改建议，不标记问题，那是 `code-review mode=check` 的职责
...
- AI 代码审查：`code-review mode=package` 生成任务包，`mode=check` 执行只读审查；understanding 产物可作为证据包补充
