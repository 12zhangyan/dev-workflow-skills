...
5. Review Gate：使用 `code-review mode=package` 基于 `docs/bugs/{date}/{task}.md` 生成审查任务包，证据包包含 Bug 文档、diff/status、复现证据和验证结果
6. 使用 `code-review mode=check` 审查任务包，把 findings 交给 `code-review mode=package` 汇总修复交接；Critical / Important 处理后重跑验证
