# yan-conversation-handoff Examples
...
4. 验证通过后使用 yan-code-review skill，mode=check，做只读审查。
...
next: 先完成 Verification Gate，再使用 yan-code-review skill，mode=check，只读审查
nextCommand: 运行聚焦测试并记录结果；通过后使用 yan-code-review skill，mode=check，审查当前实现
