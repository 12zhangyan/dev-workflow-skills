...
若用户显式提供可读的 `docs/review-form/*-review-task.md`，可直接把它作为旧版 review-task 证据；若 sibling `review-fix` 确实不可读，且工作区存在唯一、与当前任务名匹配、可读的该类文件，可把它作为生成当前任务包的兼容模板。使用前必须核对它能承载 review 范围、status/diff、需求约束、测试证据、审查轴和 finding 回收格式；缺字段按本 skill 当前规则补齐。多个同等候选时交互会话只问一次，非交互输出 `Blocked`，不得擅自选最新文件。

走兼容路径时输出 `ReviewTaskTemplateSource: legacy-docs-review-form` 和 `CompatibilityFlags: review-form-skill-missing`；它只表示旧文档模板被复用，不表示 `review-form` skill 已安装或执行。无 sibling `review-fix` 且无合格旧模板时输出 `Blocked / review-task-template-missing`，不要跨宿主继续找路径。
...
`$entry` 可为 dev-doc、Workflow Brief、review-task（含旧版 `docs/review-form/*-review-task.md`）、功能描述或“当前工作区”。未指定模式时使用 standard。
...
若 Step 0 命中合格旧版 review-task，按“宿主隔离与旧任务包兼容”复用其结构/证据并补齐当前字段；不调用不存在的 `review-form` skill，也不改变后续 review-check / review-repair / verification 流程。
...
- [ ] 已记录 ReviewTaskTemplateSource / CompatibilityFlags；未跨宿主探测 skill 路径，旧版 `docs/review-form` 仅作为校验后的模板/证据
