...
若范围内源码、测试、配置、OpenAPI 或正式 docs 仍是 Git `??` / SVN `?`，记录 `VCSGateBlocked` 并停止，不进入审查或修复。skill 不代替用户执行 `git add` / `svn add`；要求纳管后重跑。普通临时文件不属于实现范围时可排除，但必须写明依据。
