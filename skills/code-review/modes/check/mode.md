---
name: review-check
description: 对 Review 任务包、dev-doc、patch/diff 或当前工作区执行一次只读代码审查并输出结构化 findings；不得修改代码。由 code-review 根入口的 check mode 加载。
...
与相邻 skill 的分工：
- `code-review mode=package`：生成 Review 任务包，回收多方 findings，并产出修复交接。
- `code-review mode=check`：拿任务包或 diff 执行一次审查，只输出 findings，不修代码。
- `code-review mode=repair`：已有 findings 后直接修改代码并验证。
- `code-review mode=loop`：同一 AI 编排审查、修复、验证和复审闭环。
- `project-analysis mode=understanding`：生成代码地图，只梳理结构，不判断问题。
...
```text
可将以上 findings 原样交给 `code-review mode=package`，用于生成修复交接文档。
如果希望直接修复，可将 findings 交给 `code-review mode=repair`；它会修改代码并运行验证。
```
...
- [ ] 未修改任何代码或文档
- [ ] 输出可直接交给 `code-review mode=package`

## 相关资源

- 审查清单与输出模板：[reference.md](reference.md)
- 示例：[examples.md](examples.md)
- 组织多 AI review 与修复交接：`code-review mode=package`
- Review 后直接修复：`code-review mode=repair`
- Review 前代码地图：`project-analysis mode=understanding`
