...
> `code-review mode=check` 的审查清单和输出格式。该 mode 只读，不修复。
...
nextCommand: 使用 code-review skill，mode=repair，根据本输出 openFindings 中的 <CR/IM/MI ID> 直接修复并验证
...
nextCommand: 使用 project-analysis skill，mode=understanding，基于 source 和当前实现生成代码地图
...
nextCommand: 补齐缺失材料后，使用 code-review skill，mode=check，基于 source 重新执行只读审查
...
可将以上材料不足结论交给 `code-review mode=package`，用于补齐证据包。
