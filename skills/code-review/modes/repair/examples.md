...
nextCommand: 使用 code-review skill，mode=check，对当前修复执行二次只读审查
...
后续建议：
- 本次只改 Critical / Important，建议使用 `code-review mode=check` 做二次只读复查；通过后进入 `project-analysis mode=understanding` 和人工 Review。
