...
同时遵循 [../../../_shared/workflow-gates.md](../../../_shared/workflow-gates.md)：本 skill 服务 Understanding Gate；`CodeMap` 输出代码地图产物，`ImpactAnalysis` 明确写 `artifacts: 无（聊天只读分析）`；两者都给出人工 review 入口，以及如需 AI 审查时回到 `yan-code-review mode=package` / `yan-code-review mode=check` 的路径。
...
> "请选择入口方式：
> ① 功能描述（如：用户短信登录）
> ② yan-dev-doc 文档路径（如：docs/2026-06-01/sms-login.md）
> ③ 入口类或方法名（如：AuthController#login）"

检测模式（按优先级）：
- `$entry` 含 `【Workflow Brief】` → **轻量交接模式**：以 Brief 的 `changed` 文件为追踪起点，`source` 文档为方案对照来源；缺 `changed` 时回退到 `source` 指向的 yan-dev-doc 模式
- `$entry` 含 `.md` 扩展名 或以 `docs/` 开头 → **yan-dev-doc 模式**
...
> 1. [类名]（[文件路径]）
> 2. ..."
- 无候选 → 只问一个聚焦问题：让用户提供入口类、接口路径或 yan-dev-doc 路径。
...
- yan-dev-doc 模式必须生成「六、方案 vs 实现对照」章节
...
**跳过条件**：yan-dev-doc 未安装（模板目录不存在）且项目中也无 `project-html/` → 跳过本步，提示用户安装 yan-dev-doc 后可启用看板。
...
- **不越界**：不提修改建议，不标记问题，那是 `yan-code-review mode=check` 的职责
- **只做理解**：不判断缺陷或关闭 findings，不用代码地图替代审查结论
- **偏差只记录，不定性**：yan-dev-doc 模式下发现方案与实现不一致，只写"偏差/观察，Review 时关注"，不把它定为 Bug
...
- [ ] yan-dev-doc 模式已生成「六、方案 vs 实现对照」章节
...
- **必需背景：** `yan-dev-doc` skill（了解 yan-dev-doc 模式入口的文档结构）
- AI 代码审查：`yan-code-review mode=package` 生成任务包，`mode=check` 执行只读审查；understanding 产物可作为证据包补充
...
| yan-dev-doc 模式无「六、代码变更清单」 | yan-dev-doc 文档格式不同 | 切换到入口代码模式 |
| 流程图节点太多看不清 | 追踪层级过深 | 只追踪 3 层以内，更深的用「...」省略 |
| 状态机图没有生成 | 代码里没有找到明确状态跳转 | 正常，只有发现 setStatus/枚举赋值时才生成 |
| Step 4.5 找不到看板模板 | yan-dev-doc 未安装 | 跳过看板登记，提示安装 yan-dev-doc |
