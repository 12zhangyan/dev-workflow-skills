...
> 如不涉及 DB 变更，删除本节。yan-dev-doc 与后续 AI 不得执行 DDL、UPDATE 或数据修复；本节只供用户确认和授权人员审核。
>
> - **结构变更**（新增库/表/字段/索引/约束）：写建议 DDL。
> - **纯配置/提示词数据变更**（无结构变更）：保留本节，写清目标表/键/行范围、建议 UPDATE、回滚 UPDATE、只读验证 SELECT；不要为了“有 DB 动作”硬写空 DDL。

- **用户确认**：<已明确同意生成变更申请 / 待确认；待确认时作为 blocker>
- **DBA 审批状态**：<待申请 / 审批中 / 已批准；执行人必须是授权人员>
- **建议结构变更**：[新增表 / 加字段 / 加索引；仅为方案；纯数据 UPDATE 填“无”]
- **目标主键/唯一定位键**：[主键列及值来源；复合主键列全。确无主键时写有唯一约束证据的业务键；无法唯一定位时记为 blocker]
- **建议数据变更**：[配置表/提示词模板等 UPDATE 对象、按主键/唯一键写出的精确 WHERE、期望新值摘要；无则删本行]
- **影响评估**：[锁表、容量、兼容性、预计影响行数]
- **数据迁移建议**：[是否需要；不得由本 skill 执行]
- **建议回滚方案**：[如何回滚；由 DBA/授权人员审核]
- **只读验证 SQL**：[执行前后用于核对结构/数据的 SELECT / information_schema 查询]

```sql
-- 建议 DDL 或配置/提示词 UPDATE，仅供授权人员审核；不得由 yan-dev-doc 或执行 AI 直接运行
-- 数据 UPDATE 的 WHERE 必须包含完整主键；无主键时只能使用有唯一约束证据的业务键
```
...
| 文件路径 / 变更对象 | 变更类型 | 说明 |
|----------|----------|------|
| | 新增 / 修改 / 删除 / 不改 / 仅配置 UPDATE | |

> 每一行都要能指导执行：说明列必须写清"这个对象承担什么职责、改到什么程度"。修改类源码条目还必须写明为何无法用扩展替代。
>
> **纯配置/提示词变更**（零生产代码改动）时，禁止留空表或删掉本节；至少保留两行示意：
>
> | 文件路径 / 变更对象 | 变更类型 | 说明 |
> |----------|----------|------|
> | （无）生产代码 | 不改 | 本次零 Java/源码切点 |
> | `DB:<表或配置键>` / `yml|Nacos:<key>` | 仅配置 UPDATE | 更新提示词或配置值；DB 对象写清主键/唯一定位键与精确 WHERE，由授权人员执行，AI 不写库 |
