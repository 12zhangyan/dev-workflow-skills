...
本 Skill 是显式的 Plan Gate 工具：把需要先评审的需求落成文档，驱动后续编码、自测和 Code Review。它不是所有实现任务的必经步骤；用户直接要求修改代码且关键边界已有证据时，由编码 Agent 直接实施和验证，不为流程完整性强行生成 yan-dev-doc。

用户只要求理解现有代码结构、调用链、接口契约或兼容性影响，且明确不要实施方案时，使用 `yan-project-analysis mode=understanding`，不生成 yan-dev-doc 或看板条目。若这些只读结论是开发方案的前置证据，可先运行该模式，再把证据交给本 Skill 产出方案。

**一次运行的核心产物是 md 执行文档**：精确的文件路径、变更清单、约束条件、可执行 Todo，写给 Codex / Cursor / Claude Code 或开发者照着执行。

**HTML 看板是可选发布产物**：只有用户明确要求“登记/更新/发布看板”，或当前项目规则明确要求 yan-dev-doc 同步看板时才执行 Step 5.5/5.6。普通方案生成不初始化、不升级、不写入看板，也不为看板推断服务/模块归属。
...
同时遵循 [../_shared/workflow-gates.md](../_shared/workflow-gates.md)：本 skill 完成的是 Plan Gate；输出必须说明产物路径、阻塞项/冲突/假设、VCS Gate、Verification Gate、Review Gate 和后续 `yan-project-analysis mode=understanding` / 人工 review 入口。
...
当用户同时给出**一篇或多篇可读的既有 yan-dev-doc md 路径**和一段边界明确的增量描述时，先逐篇读取旧文档并启用 `IncrementalRevision`，无需重新走完整新需求问答：
...
旧文档不存在、不可读、不是 yan-dev-doc，或增量描述会改变业务/API/权限/DB 决策时，不启用快捷路径，回到标准流程并按阻塞规则处理。
...
`Compact` 只生成一份 `docs/<日期>/<任务名>.md`，使用 [精简文档模板](reference.md#精简文档模板)，保留范围/非目标、兼容性依据、两个以内改动点、验证、回滚、Todo 和评审点；跳过 Step 5.1、5.5、5.6，不生成 OpenAPI、看板、单页或索引。任一条件不满足，或预计范围扩张时立即升级为 `Standard`；给了既有 yan-dev-doc 路径时优先使用 `IncrementalRevision`。
...
若用户选"Bug 修复"且目的是记录现象、分析根因 → 提示："这类任务建议改用 yan-project-analysis skill，mode=incident（专门的 Bug 记录与根因分析流程，会登记到看板的 Bug 区）。继续用 yan-dev-doc 吗？" 用户确认继续才往下走。
...
5. 执行结果回填要求：要求实现方逐项回填 Todo 完成情况、变更文件、验证命令、偏离项；实现结束立即读取 `git status --short` / `svn status`，把新增源码、测试、配置、OpenAPI 和正式 docs 的未纳管清单显式交给用户，不得直接跳到 `yan-code-review mode=loop` 后才暴露 VCS blocker
...
7. 验证通过后 Todo：先输出 VCS checkpoint，再按 [../_shared/workflow-chain.md](../_shared/workflow-chain.md) 输出 `yan-code-review package → check → repair` 或 `loop`，随后进入 `yan-project-analysis understanding` 的真实可用调用方式；存在未纳管关键文件时必须显式列出，但 loop 仍可读取、修复、验证和复审这些文件，只能把最终 Submit/VCS Gate 结论标为阻塞；三端均可用自然语言点名 skill 与 mode，不虚构已删除的旧命令
...
- 后续步骤：`yan-code-review mode=package`（生成 Review 任务包并汇总）、`mode=check`（只读审查）、`mode=repair`（按 findings 直修）、`yan-project-analysis mode=understanding`（生成代码地图）；实际调用写法以共享 workflow-chain 为准
