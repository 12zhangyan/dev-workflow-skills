...
| 配置变更 | 配置项或提示词、影响范围、变更方式（yml/Nacos/DB UPDATE）、目标表主键/唯一定位键、回滚方案、验证方式 | yml/env/Nacos/Apollo、配置读取代码、DB 配置/提示词表及主键/唯一约束、部署文档 | 影响生产行为、需要重启、回滚方式不明确；纯配置/提示词且无代码时，变更清单必须显式写「不改」+ UPDATE 对象；UPDATE 无法用完整主键或有约束证据的唯一业务键定位时必须问 |
