...
3. **纯配置/提示词变更也要写变更清单** — 零 Java 时仍保留「六、代码变更清单」，显式写「生产代码：不改」和「仅配置 UPDATE」对象，禁止空表
4. **最小影响分析永远要写** — 这是开闭原则的硬约束
5. **风险章节务必填具体值** — "高/中/低" 后面必须有具体应对措施，不能空着
6. **代码评审关注点永远要写** — 为后续 review-check 提供具体检查目标，不能省略；调用写法按当前宿主生成
