---
name: review-repair
description: 根据 review-check / code review findings 或 review-fix 修复交接直接修改代码并验证；当用户说"review 后直接修"、"按 review 结果修复"、"修复这些 findings"、"code review 后直接改代码"、"使用 review-repair skill"时使用。此 skill 会执行修复，不是只读审查；只在已有 findings、fix-handoff、review 结果或明确问题清单时触发。用户只是要求"review/审查/看看有没有问题"时用 review-check；没有 findings 且要求同一个 AI 完成审查、修复、验证和复审时用 review-loop。
...
- 用户只给 dev-doc、patch 或功能描述但没有 findings → 不直接修；只读找问题用 `review-check`，多 AI 任务包用 `review-fix`，同一 AI 完整闭环用 `review-loop`。
