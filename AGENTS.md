...
Board writes go through **`board-add.js`** (`node project-html/board-add.js <entry.json>`). It deterministically splits one independently authored human entry into a whitelisted catalog and `data/details/<detailId>.js`, filters Agent-only fields, matches by `deliveryId`, `sourceDocPath`, then `docPath`, deep-merges lifecycle detail, updates event arrays idempotently by stable `eventId`, preserves governance state, maintains `updatedAt`, backs up `changes.js`, and aborts on parse/count regressions. Before writing, `node project-html/board-add.js --check-migration` reports whether the catalog needs conversion. If it reports `BOARD_DATA_MIGRATION_REQUIRED`, run `node project-html/board-add.js --migrate` and recheck before writing; migration converts legacy rich entries without reading md. The template catalog ships empty.
