...
| 记录 Bug | 自然语言点名 `project-analysis mode=incident` | `使用 project-analysis skill，mode=incident，记录 Bug名` |
| 梳理测试业务流 | 自然语言点名 `project-analysis mode=business` | `使用 project-analysis skill，mode=business，生成业务流方案` |
| 生成 Review 任务包 | 自然语言点名 `code-review mode=package` | `使用 code-review skill，mode=package，基于 docs/.../任务.md 生成 Review 任务包` |
| 执行只读审查 | 自然语言点名 `code-review mode=check` | `使用 code-review skill，mode=check，审查 docs/review-fix/...-review-task.md` |
| Review 后直接修复 | 自然语言点名 `code-review mode=repair` | `使用 code-review skill，mode=repair，根据这些 findings 直接修复` |
| 单 AI 一键 Review 闭环 | 自然语言点名 `code-review mode=loop` | `使用 code-review skill，mode=loop，审查、修复、验证并复审当前工作区` |
| 生成代码地图 | 自然语言点名 `project-analysis mode=understanding` | `使用 project-analysis skill，mode=understanding，生成代码地图` |
| 只读判断接口与调用链影响 | 自然语言点名 `project-analysis mode=understanding` | `使用 project-analysis skill，mode=understanding，执行 ImpactAnalysis，不生成文档或看板` |

Codex 不要输入 `/dev-doc` 或 `$dev-doc`。Codex 安装 skill 不等于注册同名斜杠命令，也不保证进入 `$` 技能选择器。

Cursor 使用当前 skill 入口或自然语言点名；例如：`使用 code-review skill，mode=loop，基于 docs/.../任务.md 审查、修复、验证并复审当前工作区`。若宿主没有结构化提问工具，按共享交互策略降级为聊天单选，不依赖固定工具名。
...
| 门禁 | 必须看到的证据 | 通过后进入 |
|------|----------------|------------|
| Plan Gate | dev-doc / bug-fix / biz-flow 文档；阻塞项、冲突、假设已写清；dev-doc 仅在明确请求/项目规则要求时发布看板，Compact 明确标记派生产物 NotApplicable | 实现 |
...
```text
使用 code-review skill，mode=check，审查 docs/review-fix/2026-07-09/xxx-review-task.md。
先读下面的 Workflow Brief，再按 tokenHint 读取任务包和 changed 文件；不要要求我粘贴全文。
```

```text
使用 code-review skill，mode=repair，根据这些 findings 直接修复。
...
```text
Claude Code: 使用 code-review skill，mode=package，基于 docs/YYYY-MM-DD/<task>.md 生成 Review 任务包
Codex: 使用 code-review skill，mode=package，基于 docs/YYYY-MM-DD/<task>.md 生成 Review 任务包
```
...
```text
Claude Code: 使用 code-review skill，mode=check，审查 docs/review-fix/YYYY-MM-DD/<task>-review-task.md
Codex: 使用 code-review skill，mode=check，审查 docs/review-fix/YYYY-MM-DD/<task>-review-task.md
```
...
```text
Claude Code: 使用 code-review skill，mode=repair，处理 <粘贴findings或fix-handoff路径>
Codex: 使用 code-review skill，mode=repair，根据这些 findings 直接修复
```
...
```text
Claude Code: 使用 code-review skill，mode=loop，基于 docs/YYYY-MM-DD/<task>.md 完成闭环
Codex: 使用 code-review skill，mode=loop，基于 docs/YYYY-MM-DD/<task>.md 审查、修复、验证并复审当前工作区
```
...
```text
Claude Code: 使用 project-analysis skill，mode=understanding，基于 docs/YYYY-MM-DD/<task>.md 生成代码地图
Codex: 使用 project-analysis skill，mode=understanding，基于 docs/YYYY-MM-DD/<task>.md 生成代码地图
```
